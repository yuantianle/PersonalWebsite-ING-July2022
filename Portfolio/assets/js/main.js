...
var Grid = (function() {
	...
	function showPreview( $item ) {
		...
		$grid.parents('.collapse').attr('data-preview', 'true');
	...
	function hidePreview() {
		...
		setTimeout(() => {
			$grid.parents('.collapse').attr('data-preview', 'false');
		}, 300)
	}
	...
	Preview.prototype = {
		...
		positionPreview : function() {
			...
			// case 3 : preview height + item height does not fit in window´s height and preview height is bigger than window´s height
																		/*added for the collapse of the pictures [finished debug] */			
																		console.log(this.$item.offset().top);																												
			var position = this.$item.data( 'offsetTop' ),
				previewOffsetT = this.$previewEl.offset().top - scrollExtra,
				scrollVal = this.height + this.$item.data( 'height' ) + marginExpanded <= winsize.height ? position : this.height < winsize.height ? previewOffsetT - ( winsize.height - this.height ) : previewOffsetT;
			
			/*added for the collapse of the pictures [finished debug] */ 
			scrollVal =  this.$item.offset().top - $(window).height()/2 + 270;
...
var pGrid = (function() {
	...
	function initItemsEvents( $items ) {
		$items.on( 'click', 'span.pg-close', function() {
			hidePreview();
			return false;
		} ).children( 'a' ).on( 'click', function(e) {
	...
	function showPreview( $item ) {
		...
		var preview = $.data( this, 'preview' ),
			...
			/*[picture collapse debug finished]*/
			position = $item.offset().top;
	...
	Preview.prototype = {
		create : function() {
			...
			this.$description = $( '<p></p>' );
			this.$href = $();
			this.$details = $( '<div class="pg-details"></div>' ).append( this.$title, this.$description, this.$href );
			this.$loading = $( '<div class="pg-loading"></div>' );
			this.$fullimage = $( '<div class="pg-fullimg"></div>' ).append( this.$loading );
			this.$closePreview = $( '<span class="pg-close"></span>' );
			this.$previewInner = $( '<div class="pg-expander-inner"></div>' ).append( this.$closePreview, this.$fullimage, this.$details );
			this.$previewEl = $( '<div class="pg-expander"></div>' ).append( this.$previewInner );
			// append preview element to the item
			this.$item.append( this.getEl() );
			// set the transitions for the preview and the item
			if( support ) {
		...
		update : function( $item ) {
			...
			if( current !== -1 ) {
				var $currentItem = $items.eq( current );
				$currentItem.removeClass( 'pg-expanded' );
				this.$item.addClass( 'pg-expanded' );
				// position the preview correctly
				this.positionPreview();
			}
		...
		close : function() {
			...
			var self = this,
				onEndFn = function() {
					...
					self.$item.removeClass( 'pg-expanded').css('height','');
					self.$previewEl.remove();
				};
		...
		setHeights : function() {
			...
			var self = this,
				onEndFn = function() {
					...
					self.$item.addClass( 'pg-expanded' );
				};
...
class TextScramble {
  constructor(el) {
    this.el = el
    this.chars = '010101110001010101010110'//'!<>-_\\/[]{}—=+*^?#________'
...
function ssc_wheel(e) {
	//e.preventDefault();
	//window.addEventListener('wheel', null, {passive:false});
